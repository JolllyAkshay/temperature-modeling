"""
Fetch historical ISO-NE load + ERA5 temperatures, train and save the ISO-NE model.

Usage:
    python collect_isone_load.py           # fetch data + train
    python collect_isone_load.py --retrain # force retrain even if model exists
"""
...
from temperature_modeling.isone_load import (
    _ISONE_MODEL_PATH,
    build_isone_training_data,
    fetch_isone_load_daily,
)
...
TRAINING_DATA_PATH = _HERE / "api_cache" / "isone_load_training.json"
...
def main():
    ...
    print(f"\n[1/4] Fetching ISO-NE (ISNE) daily load  {start} → {end} ...")
    load_daily = fetch_isone_load_daily(start, end, session)
    print(f"      {len(load_daily)} days fetched")

    print(f"\n[2/4] Fetching ERA5 for {len(ISONE_LOAD_LOCATIONS)} ISO-NE locations ...")
    era5_avg, era5_hilo = {}, {}
    for i, loc in enumerate(ISONE_LOAD_LOCATIONS, 1):
        label = loc["label"]
        coords = Coordinates(lat=loc["lat"], lon=loc["lon"])
        print(f"      [{i}/{len(ISONE_LOAD_LOCATIONS)}] {label} ...", end=" ", flush=True)
    ...
    observations = build_isone_training_data(load_daily, era5_avg, era5_hilo)
    ...
    if Path(_ISONE_MODEL_PATH).exists() and not retrain:
        print(f"\n[4/4] Model exists at {_ISONE_MODEL_PATH}. Pass --retrain to overwrite.")
        return

    print(f"\n[4/4] Training XGBoost ISO-NE model on {len(observations)} observations ...")
    ...
    save_load_model(model, _ISONE_MODEL_PATH)
    print(f"      Saved: {_ISONE_MODEL_PATH}")
    print("\nDone.")
