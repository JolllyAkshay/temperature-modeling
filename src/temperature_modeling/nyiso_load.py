"""
NYISO electricity load forecasting from temperature projections.
Mirrors caiso_load.py but targets the NYIS EIA respondent (New York ISO).
"""
...
from .nyiso import NYISO_LOAD_LOCATIONS
...
_NYISO_MODEL_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "api_cache", "nyiso_load_model.pkl"
)

_EIA_RESPONDENT = "NYIS"
...
def fetch_nyiso_load_daily(start: date, end: date, session: requests.Session) -> Dict[date, float]:
    """Fetch NYISO hourly demand from EIA and aggregate to daily mean (MW)."""
    all_rows: list = []
    offset = 0
    page_size = 5000
        ...
        if cached is not None:
            ...
            for attempt in range(5):
                try:
                    ...
                    if attempt == 4:
                        raise RuntimeError(f"EIA NYISO fetch failed: {exc}") from exc
...
def weighted_avg_temp_f_nyiso(temps_c_by_label: Dict[str, float]) -> float:
    ...
    for loc in NYISO_LOAD_LOCATIONS:
        ...
        total_w  += w
    if total_w == 0:
        raise ValueError("No NYISO location temperatures available")
    return total_wt / total_w


def build_nyiso_training_data(
...
_NYISO_COMPARISON_CACHE = os.path.join(
    os.path.dirname(__file__), "..", "..", "api_cache", "nyiso_comparison_cache.json"
)


def fetch_nyiso_official_comparison(
    session: requests.Session,
    lookback_days: int = 14,
) -> Dict[str, Dict]:
    """
    Fetch EIA actual demand (D) and day-ahead forecast (DF) for NYISO (NYIS).
    Returns {"actual": {date_str: gw}, "da_fcst": {date_str: gw}}.
    """
    import json as _json

    today = date.today()
    if os.path.exists(_NYISO_COMPARISON_CACHE):
        try:
            cached = _json.loads(open(_NYISO_COMPARISON_CACHE).read())
            age_h = (time.time() - os.path.getmtime(_NYISO_COMPARISON_CACHE)) / 3600
    ...
    try:
        os.makedirs(os.path.dirname(_NYISO_COMPARISON_CACHE), exist_ok=True)
        open(_NYISO_COMPARISON_CACHE, "w").write(_json.dumps(result))
    except Exception:
        pass
    return result
