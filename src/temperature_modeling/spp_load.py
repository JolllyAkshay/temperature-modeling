...
def build_spp_training_data(
    ...
    for d in sorted(load_daily):
        ...
        if dew_c:
            dewpoint_hi_by_date[d] = weighted_avg_temp_f_spp(dew_c)
        if wind_kph:
            total_w = sum(loc["weight"] for loc in SPP_LOAD_LOCATIONS if loc["label"] in wind_kph)
            avg_kph = sum(loc["weight"] * wind_kph[loc["label"]]
                          for loc in SPP_LOAD_LOCATIONS if loc["label"] in wind_kph) / total_w
