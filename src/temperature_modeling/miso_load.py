...
def build_miso_training_data(
    ...
    for d in sorted_dates:
        ...
        if dew_c:
            dewpoint_hi_by_date[d] = weighted_avg_temp_f_miso(dew_c)
        if wind_kph:
            total_w = sum(loc["weight"] for loc in MISO_LOAD_LOCATIONS if loc["label"] in wind_kph)
            avg_kph = sum(loc["weight"] * wind_kph[loc["label"]]
                          for loc in MISO_LOAD_LOCATIONS if loc["label"] in wind_kph) / total_w
